...
public class Juliaexp2 implements Fractal {
	...
	public Juliaexp2(double c_real, double c_imaginary) throws IOException {
		c.setReal(c_real);
	...
	public void createImage() {
		...
		try {
			ImageIO.write(image, "png", new File("juliaexp2.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public int testPoint(Complex z) {
		...
		for (int i = 0; i < 100 * colours.length; i++) {
			// Compute z = z*z + c;
	...
	public static void main(String[] args) throws IOException {
		Fractal juliatest = new Juliaexp2(-0.65, 0);
	...
	public void nextIteration(Complex z, Complex constant) {
		...
		z.setReal(Math.exp(r)*Math.cos(i)
				+ constant.getReal());
		z.setImaginary(Math.exp(r)*Math.sin(i)
